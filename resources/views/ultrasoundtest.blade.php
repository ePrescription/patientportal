...
<form action="{{URL::to('/')}}/lab/rest/patient/ultrasoundtestresults" role="form" method="POST" onsubmit="return submitForm(this);" class="form-horizontal" id="ultrasound">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-4">
            <input type="text" class="form-control" name="examinationDate" id="usTestDate" value="{{date('Y-m-d')}}" style="line-height: 20px;" required="required" onchange="changeTimeSlots(this.value);" />
            @if ($errors->has('examinationDate'))<p class="error" style="">{!!$errors->first('examinationDate')!!}</p>@endif
        </div>
        <div class="col-sm-4">
            <select class="form-control" name="examinationTime" id="usexaminationTime"
                    required="required">
    ...
    @foreach($patientUltraSoundTests as $patientUltraSoundTestsValue)
        <div class="col-sm-6 form-group">
            <label class="col-sm-6 control-label">{{$patientUltraSoundTestsValue->examination_name}}</label>
            <div class="col-sm-6">
