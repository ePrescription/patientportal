...
<script>
    ...
    $('#motTestDate').datepicker({
        dateFormat: "mm/dd/yy",
        minDate: new Date()
    });

    function changeValues(did) {
        var dateValue = $("#motTestDate").val();
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.result['result'] == "Doctor Is Not Available") {
                        ...
                        $("#motionexaminationTime").html(terms);
                    } else {
                        var terms = '<option value="">--Choose Time--</option>';
    ...
    function submitForm() {
        ...
        if(sthospitalId==""&&hospitalId==""){
            ...
            var list;
            if(hospitalId==""){
                list1 = list1 + "<option value='" + sthospitalId + "'>" + sthospitalId + "</option>";
                $('#hospitalId').html(list1);
            }else{
                list1 = list1 + "<option value='" + hospitalId + "'>" + hospitalId + "</option>";
                $('#hospitalId').html(list1);
            }
            if(doctorId==""){
                list = list + "<option value='" + stdoctorId + "'>" + stdoctorId + "</option>";
                $('#doctorId').html(list);
            }else{
                list = list + "<option value='" + doctorId + "'>" + doctorId + "</option>";
                $('#doctorId').html(list);
            }
            return confirm('Do you really want to submit the form?');
        }
...
<form action="{{URL::to('/')}}/lab/rest/patient/motiontestresults" role="form" method="POST" onsubmit="return submitForm(this);" class="form-horizontal " id="motion">
    <div class="form-group">

    <div class="form-group">
        <label class="col-sm-4 control-label">Select Hospital</label>
        <div class="col-sm-8">
            <select name="hospitalId" id="hospitalId" onchange="loaddoctor(this.value)"  class="form-control input-md mothospId" placeholder="Hospital">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-4">
            <input type="text" class="form-control" name="examinationDate" id="motTestDate" value="{{date('Y-m-d')}}" style="line-height: 20px;" required="required" onchange="javascript:changeTimeSlots(this.value);" />
            @if ($errors->has('examinationDate'))<p class="error" style="">{!!$errors->first('examinationDate')!!}</p>@endif
        </div>
        <div class="col-sm-4">
            <select class="form-control" name="examinationTime" id="motionexaminationTime"
                    required="required">
    ...
    @foreach($patientMotionTests as $patientMotionTestsValue)
        <div class="col-sm-6 form-group">
            <label class="col-sm-6 control-label">{{$patientMotionTestsValue->examination_name}}</label>
            <div class="col-sm-6">
                ...
                <div class="radio radio-info col-sm-6 control-label">
